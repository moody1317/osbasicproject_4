...
api_url = "https://open.assembly.go.kr/portal/openapi/nzgjnvnraowulzqwl"  # API 주소
            ...
            if isinstance(api_data, list) and len(api_data) > 1:
                ...
                if isinstance(second_item, dict) and 'row' in second_item:
                    ...
                    for i, item in enumerate(filtered_data):
                        print(f"{i+1}. BILL_ID: {item['BILL_ID']}, AGE: {item['AGE']}")
                    ...
                    with open("bill_age_data.json", "w", encoding="utf-8") as f:
                        json.dump(output_data, f, ensure_ascii=False, indent=2)
                    print(f"\n{len(filtered_data)}개의 정보가 'bill_age_data.json' 파일로 저장되었습니다.")
                else:

...
async function loadAllData() {
    try {
        ...
        const [membersResult, performanceResult, photosResult, rankingResult] = results;
        ...
        if (performanceResult.status === 'fulfilled') {
            ...
            console.log(`[RankMember] ✅ 실적 데이터: ${pageState.memberPerformance.length}개`);
        } else {
            console.warn('[RankMember] ⚠️ 실적 데이터 로드 실패:', performanceResult.reason);
            pageState.memberPerformance = [];
        }
        
        // 사진 데이터 (선택)
        if (photosResult.status === 'fulfilled') {
            pageState.memberPhotos = photosResult.value || [];
            console.log(`[RankMember] ✅ 사진 데이터: ${pageState.memberPhotos.length}개`);
        } else {
            console.warn('[RankMember] ⚠️ 사진 데이터 로드 실패:', photosResult.reason);
            pageState.memberPhotos = [];
        }
...
function mergeAndSortData() {
    try {
        // 의원 명단을 기본으로 하여 다른 데이터들을 병합
        pageState.filteredMembers = pageState.memberList.map((member, index) => {
            const memberName = member.HG_NM || member.name || '';
            
            // 실적 데이터 찾기
            const performance = pageState.memberPerformance.find(p => 
                (p.lawmaker_name || p.name) === memberName
            );
            
            // 랭킹 데이터 찾기
            const ranking = pageState.memberRanking.find(r => 
                (r.lawmaker_name || r.HG_NM || r.name) === memberName
            );
            
            // 사진 데이터 찾기
            const photo = pageState.memberPhotos.find(ph => 
                (ph.member_name || ph.name) === memberName
            );
            
            return {
                rank: ranking ? parseInt(ranking.rank) || (index + 1) : (index + 1),
                name: memberName,
                party: member.POLY_NM || member.party || '정당 없음',
                contact: member.tel_no || member.contact || '',
                homepage: member.homepage || member.home_page || '',
                totalScore: performance ? parseFloat(performance.total_score) || 0 : 0,
...
function renderTable() {
    ...
    const tableHTML = currentPageMembers.map(member => `
        <tr>
            ...
            <td class="phone-number">${member.contact || '연락처 없음'}</td>
            <td class="home-icon">
...
window.rankMemberDebug = {
    ...
    showInfo: () => {
        ...
        console.log(`- 실적 데이터: ${pageState.memberPerformance.length}개`);
        console.log(`- 랭킹 데이터: ${pageState.memberRanking.length}개`);
        console.log(`- 사진 데이터: ${pageState.memberPhotos.length}개`);
    }
};
